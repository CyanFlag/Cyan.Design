...
/// <summary>时间输入框控件，支持时分秒和 12/24 小时制切换</summary>
...
public class CyanTimeInput : TemplatedControl
{
    /// <summary>当前时间值样式属性</summary>
    public static readonly StyledProperty<TimeSpan?> ValueProperty =
        AvaloniaProperty.Register<CyanTimeInput, TimeSpan?>(nameof(Value),
            defaultBindingMode: BindingMode.TwoWay, enableDataValidation: true);

    /// <summary>当前时间值</summary>
    ...
    /// <summary>小时字符串样式属性</summary>
    public static readonly StyledProperty<string> HourStringProperty =
        AvaloniaProperty.Register<CyanTimeInput, string>(nameof(HourString), "00");

    /// <summary>小时字符串</summary>
    ...
    /// <summary>分钟字符串样式属性</summary>
    public static readonly StyledProperty<string> MinuteStringProperty =
        AvaloniaProperty.Register<CyanTimeInput, string>(nameof(MinuteString), "00");

    /// <summary>分钟字符串</summary>
    ...
    /// <summary>秒字符串样式属性</summary>
    public static readonly StyledProperty<string> SecondStringProperty =
        AvaloniaProperty.Register<CyanTimeInput, string>(nameof(SecondString), "00");

    /// <summary>秒字符串</summary>
    ...
    /// <summary>时钟制式标识样式属性（12HourClock 或 24HourClock）</summary>
    public static readonly StyledProperty<string> ClockIdentifierProperty =
        AvaloniaProperty.Register<CyanTimeInput, string>(nameof(ClockIdentifier), "12HourClock",
            coerce: CoerceClockIdentifier);

    /// <summary>时钟制式标识（12HourClock 或 24HourClock）</summary>
    ...
    private static string CoerceClockIdentifier(AvaloniaObject sender, string value)
    {
        ...
        {
            throw new ArgumentException("Invalid ClockIdentifier", nameof(ClockIdentifier));
        }
        return value;
    }

    /// <summary>是否启用秒输入样式属性</summary>
    public static readonly StyledProperty<bool> UseSecondsProperty =
        AvaloniaProperty.Register<CyanTimeInput, bool>(nameof(UseSeconds));

    /// <summary>是否启用秒输入</summary>
    ...
    /// <summary>输入框是否聚焦样式属性</summary>
    public static readonly StyledProperty<bool> InputFocusProperty =
        AvaloniaProperty.Register<CyanTimeInput, bool>(nameof(InputFocus));

    /// <summary>输入框是否聚焦</summary>
    ...
    private void OnInputChanged(object? sender, TextChangedEventArgs e)
    {
        ...
        if ((textBox.Text?.Length ?? 0) < 2) return;
        ...
        if (textBox.Name == "PART_HourTextBox")
        {
            ...
            switch (ClockIdentifier)
            {
                ...
                case "12HourClock" when value == 0:
                    if (_toggleButton != null) _toggleButton.IsChecked = false;
                    textBox.Text = "00";
                    Value = new TimeSpan(value, Value?.Minutes ?? 0, Value?.Seconds ?? 0);
                    return;
            }
    ...
    private void OnToggleButtonCheckChanged(object? sender, RoutedEventArgs e)
    {
        ...
        if (toggleButton.IsChecked == true)
        {
            ...
            if (ClockIdentifier == "12HourClock" && _hourTextBox is { Text: "00" }) _hourTextBox.Text = "12";
        }
        else
        {
            if (Value is { Hours: >= 12 })
            ...
            if (ClockIdentifier == "12HourClock" && _hourTextBox is { Text: "12" }) _hourTextBox.Text = "00";
        }
    }
    ...
    private void UpdateHour(AvaloniaPropertyChangedEventArgs args)
    {
        ...
        if (ClockIdentifier == "12HourClock" && value < 12 && _toggleButton is { IsChecked: true }) value += 12;
